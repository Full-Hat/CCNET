...
int cash_code_protocol::enable_sequence() {
    ...
    if(!this->m_is_connected) {
        logger::get_instance()->write("COM port is not opened", LOGGER_MSG_SOURCE, logger::ERROR);
...
int cash_code_protocol::validator_listener() {
    ...
    if(result[3] != 0x14) {
        if(result[3] == 0x15) {
            ...
            /****** ЗДЕСЬ В ДАЛЬНЕЙШЕМ БУДЕТ ПРОВЕРКА СОСТОЯНИЯ СЕТИ ******/
...
bool cash_code_protocol::validator_listener_without_bill(int& amount) {
    vec_bytes result;
    ...
    amount = this->cash_code_table(result[4]) / 10;
    ...
    // Проверка на ошибки, если были обнаружены ошибки посылаем команду NAK
    if(this->check_errors(result)) {
        logger::get_instance()->write("discovered some errors. sending the NAK command", LOGGER_MSG_SOURCE, logger::ERROR);
        this->send_command(validator_commands_CCNET::NAK);
        //std::cout << "NAK 2\n";
        return false;
    }
    ...
    if(result[3] != 0x14) {
        ...
        if(result[3] == 0x15) {
            ...
            // STACK
            //result = this->send_command(validator_commands_CCNET::STACK);
            std::cout << "Trying to hold the bill\n";
            this->send_command(validator_commands_CCNET::HOLD);
            return true;
        }
        else if(result[3] == 0x17) {
...
void cash_code_protocol::stack_bill() {
    this->send_command(validator_commands_CCNET::STACK);
} 

void cash_code_protocol::unstack_bill() {
    this->send_command(validator_commands_CCNET::ACK);
}

void cash_code_protocol::hold_bill() {
    this->send_command(validator_commands_CCNET::HOLD);
}
 ...
 */
vec_bytes cash_code_protocol::send_command(validator_commands_CCNET cmd, vec_bytes data) {
    vec_bytes response;

    if(validator_commands_CCNET::ACK == cmd || validator_commands_CCNET::NAK == cmd) {
        vec_bytes bytes;
        if(cmd == validator_commands_CCNET::ACK)
            bytes = pack.create_response(validator_commands_CCNET::ACK);
        if(cmd == validator_commands_CCNET::NAK)
            bytes = pack.create_response(validator_commands_CCNET::NAK);
...
{
    try {
        if(this->com_port->start(port_path.c_str(), 19200)) { // 9600
