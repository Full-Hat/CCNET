...
int CashCodeProtocol::EnableSequence()
{
    ...
    this->SendCommand(ValidatorCommands::ENABLE_BILL_TYPES, ENABLE_BILL_TYPES_WITH_ESCROW);
    return 0;
}
        ...
        while(true)
        {
            ...
            if(result[3] != 0x14)
            {
                ...
                else if(result[3] == 0x81)
                {
                    ...
                    m_CashReceived += this->CashCodeTable(result[4]) / 10;
                    std::cout << "\nCASH: " << std::dec << this->CashCodeTable(result[4]) / 10 << std::endl;
...
std::string CashCodeProtocol::FindPort()
{
    const int MAX_PORT_NUMBER = 31;
    for(int i = 0; i < MAX_PORT_NUMBER; ++i)
    {
        std::string portPath = std::string("/dev/ttyS") + std::to_string(i);
        int err = ConnectValidator(portPath.c_str());
        ...
        std::cout << "Found device :: portPath=" << portPath << "\n";
        return portPath;
    }
 ...
 */
int CashCodeProtocol::ConnectValidator(std::string portPath)
{
    try
    {
        if(this->ComPort->start(portPath.c_str(), 9600)) {
...
bool CashCodeProtocol::CheckErrors(vec_bytes &result)
{
    ...
    case 0x1C:
        switch (result[4]) {
            ...
            case 0x68: m_LastError = 0x68; break; // this error
